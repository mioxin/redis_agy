...
### ADR-002: Реактивная модель отслеживания (Reactive On-Demand Tracking) с предзагрузкой кэша при старте
- **Контекст**: Список активных заказов известен не только клиенту, но и серверу (через `orders.yml`). При старте сервер должен заполнить кэш до первого клиентского запроса, исключая Cache Miss для известных заказов.
- **Решение**: Совмещение двух моделей:
  - **Pre-Warm на старте**: при запуске сервера выполняется безусловная предзагрузка всех заказов из `orders.yml` в кэш (см. ADR-002 Supplement).
  - **Reactive On-Demand**: заказы, отсутствующие в `orders.yml`, берутся на мониторинг **только в момент первого запроса** `GET /orders/{orderId}/courier-location`.
  ...
  - **Тестовая реализация**: предзагрузка используется только в тестах/моках. В production заказы будут известны при создании клиентом — кэш заполняется на этапе создания заказа, а не при старте сервера.

### ADR-002 (Supplement): Предзагрузка кэша при старте (Pre-Warm Cache)
- **Контекст**: Список активных заказов известен серверу (через `orders.yml`). Кэш должен быть заполнен при старте, до первого запроса клиента, чтобы исключить Cache Miss для известных заказов.
- **Решение**: При старте сервера выполняется безусловная предзагрузка кэша. Поток предзагрузки реализован методом `PollerService.PreWarmCache(ctx) error`, который:
  1. Получает все заказы через `OrderServiceMock.GetAllOrders()`.
  2. Вызывает `CacheRepository.PreWarmOrders(ctx, orders, heartbeatTTL, mappingTTL) error` — единая точка записи: регистрация в `tracking:active_orders` + инициализация heartbeats + кэширование маппинга `order -> courier_id` в одном Redis Pipeline.
  3. Группирует заказы по `courier_id` (дедупликация).
  4. Конкурентно (с семафором `WorkerConcurrency`) опрашивает Courier Service для каждого уникального курьера.
  5. Пакетно сохраняет локации через существующий `SetOrderLocationsBatch` (Pipeline, TTL 120 с).
- **Мультипод-координация**: Используются методы `CacheRepository.SetPreWarmed(ctx) error` (запись флага `courier_poller:prewarmed` в Redis) и `CacheRepository.IsPreWarmed(ctx) (bool, error)` (чтение флага). Первый запущенный под выполняет предзагрузку и устанавливает флаг; остальные поды проверяют флаг через `IsPreWarmed` и пропускают предзагрузку.
- **Тестовая реализация**: Предзагрузка является **тестовой** и используется только в текущей реализации с моками. В production-сценарии предзагрузка **не требуется** — заказы будут известны при создании клиентом (кэш заполняется на этапе создания заказа, а не при старте сервера). Флаг `courier_poller:prewarmed` и методы `SetPreWarmed`/`IsPreWarmed` также не потребятся в production.
...
| courier_poller:prewarmed           | String     | -       | Флаг предзагрузки (SetPreWarmed/IsPreWarmed, multi-pod coordination) |
+------------------------------------+------------+---------+--------------------+
```
